...
async function initSchema() {
  ...
  // Migracja: dodaj kolumny śledzenia odczytu (idempotentnie)
  await sql`ALTER TABLE conversations ADD COLUMN IF NOT EXISTS last_read_a TIMESTAMPTZ`;
  await sql`ALTER TABLE conversations ADD COLUMN IF NOT EXISTS last_read_b TIMESTAMPTZ`;
  ...
  // Migracja: kolumny śledzenia rozwiązania zgłoszeń
  await sql`ALTER TABLE reports ADD COLUMN IF NOT EXISTS resolved_at TIMESTAMPTZ`;
  await sql`ALTER TABLE reports ADD COLUMN IF NOT EXISTS resolved_by TEXT REFERENCES users(id) ON DELETE SET NULL`;
  await sql`ALTER TABLE reports ADD COLUMN IF NOT EXISTS resolution_note TEXT`;
