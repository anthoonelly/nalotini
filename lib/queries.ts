...
export async function createReservation(input: {
  ...
  // Jeśli jest wiadomość, utwórz konwersację i wyślij ją automatycznie
  if (input.message && input.message.trim().length > 0) {
    const conversationId = await getOrCreateConversation({
      userId: input.userId,
      otherUserId: input.authorId,
...
export async function resolveReport(id: string, resolvedBy: string, note?: string) {
  await ensureDbReady();
  await sql`
    UPDATE reports SET
      status = 'resolved',
...
export async function markConversationRead(conversationId: string, userId: string) {
  await ensureDbReady();
  // Conversations have user_a and user_b; update the matching column
  await sql`
    UPDATE conversations SET
      last_read_a = CASE WHEN user_a = ${userId} THEN NOW() ELSE last_read_a END,
      last_read_b = CASE WHEN user_b = ${userId} THEN NOW() ELSE last_read_b END
...
export async function getUnreadMessageCount(userId: string): Promise<number> {
  ...
  const rows = await sql`
    ...
    WHERE m.sender_id != ${userId}
      ...
      AND (
        (c.user_a = ${userId} AND (c.last_read_a IS NULL OR m.created_at > c.last_read_a))
        OR
        (c.user_b = ${userId} AND (c.last_read_b IS NULL OR m.created_at > c.last_read_b))
      )
  `;
  return Number(rows[0]?.cnt ?? 0);
}

export async function getPendingReservationsCount(authorId: string): Promise<number> {
  await ensureDbReady();
  const rows = await sql`
    SELECT COUNT(*)::int AS cnt
    FROM reservations r
...
// Wzbogać konwersacje o flagę "unread" dla użytkownika
export async function getUserConversationsWithUnread(userId: string) {
  await ensureDbReady();
  const rows = await sql`
    SELECT
      c.id,
      ...
      (
        ...
        WHERE m.conversation_id = c.id
          ...
          AND m.created_at > COALESCE(
            CASE WHEN c.user_a = ${userId} THEN c.last_read_a ELSE c.last_read_b END,
            'epoch'::timestamptz
          )
      ) AS unread_count
    FROM conversations c
